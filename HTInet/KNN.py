...
def main(options):
    ...
    Wfile=open(r"./n2vnew_1/result/KNN/result.txt","a")
    Wfile1=open(r"./n2vnew_1/result/KNN/mean_fpr.txt","a")
    for i in range(options.fold_num):
        ...
        #best_RandomF = RandomForestClassifier(n_estimators=options.n_neighbors)
        best_RandomF=KNeighborsClassifier(n_neighbors=options.n_neighbors)
        ...
        dataframe0.to_csv(r'./n2vnew_1/result/KNN/real_predict'+str(i)+'.csv',columns=['real','predict'],header=None,index=False)  
        
        fpr,tpr,thresholds=roc_curve(Y_test,y_score_RandomF[:,1])
        
        data={'fpr':fpr,'tpr':tpr,'thresholds':thresholds}
        dataframe=pd.DataFrame(data)
        dataframe.to_csv(r'./n2vnew_1/result/KNN/fpr_tpr'+str(i)+'.csv',columns=['fpr','tpr','thresholds'],header=None,index=False)
        
        pre,re,thresholds=precision_recall_curve(Y_test,y_score_RandomF[:,1])
        #print()
        data1={'precision':pre,'recall':re}
        dataframe1=pd.DataFrame(data1)
        dataframe1.to_csv(r'./n2vnew_1/result/KNN/precision_recall'+str(i)+'.csv',columns=['precision','recall'],header=None,index=False)
...
if __name__ == '__main__':
    ...
    parser.add_option('-n', '--n_neighbors', action='store',
                      dest='n_neighbors', default=5, type='int',
